...
class move_pillbottle_pad_left(Base_Task):

    def setup_demo(self, **kwags):
        super()._init_task_env_(**kwags)

    def load_actors(self):
        # Force pillbottle to be on the left side (x < 0)
        # Avoid the very center area (abs(x) < 0.10) to ensure distinct sides
        rand_pos = rand_pose(
            xlim=[-0.25, -0.12],
            ylim=[-0.1, 0.1],
        ...
        # No need for while loop since xlim is already constrained
        ...
        # Force pad to be on the left side too
        xlim = [-0.25, -0.12]
