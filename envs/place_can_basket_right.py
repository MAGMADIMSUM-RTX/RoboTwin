...
class place_can_basket_right(Base_Task):

    def setup_demo(self, is_test=False, **kwags):
        super()._init_task_env_(**kwags)

    def load_actors(self):
        # Force to use right arm
    ...
    def play_once(self):
        # Grasp the can with the right arm
        ...
        if self.plan_success is False:
            ...
            # Return current arm to origin
            self.move(self.back_to_origin(arm_tag=self.arm_tag))
            
            # Use the SAME arm to grasp the basket.
            self.move(self.grasp_actor(self.basket, arm_tag=self.arm_tag, pre_grasp_dis=0.02, contact_point_id=1))
        else:
            # Open the gripper to release the can
            self.move(self.open_gripper(arm_tag=self.arm_tag))
            ...
            # Use the SAME arm to grasp the basket.
            self.move(self.grasp_actor(self.basket, arm_tag=self.arm_tag, pre_grasp_dis=0.08, contact_point_id=1))

        # Close the gripper to firmly grasp the basket
        self.move(self.close_gripper(arm_tag=self.arm_tag))
        # Lift and slightly pull the basket inward
        self.move(
            self.move_by_displacement(arm_tag=self.arm_tag,
                                      x=0.02,
                                      z=0.05))
