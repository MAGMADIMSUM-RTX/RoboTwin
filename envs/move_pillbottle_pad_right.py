...
class move_pillbottle_pad_right(Base_Task):

    def setup_demo(self, **kwags):
        super()._init_task_env_(**kwags)

    def load_actors(self):
        # Force pillbottle to be on the right side (x > 0)
        # Avoid the very center area (abs(x) < 0.12)
        rand_pos = rand_pose(
            xlim=[0.12, 0.25],
            ylim=[-0.1, 0.1],
        ...
        # Force pad to be on the right side too
        xlim = [0.12, 0.25]
